class GoogleMeet {
	static DajZadnjePredavanje(s) {
		...
		var div = doc.getElementsByClassName('course-content')[0]; // prvi el koji ima klasu course-content
		if (div === undefined) return rez;
		var ul = div.getElementsByClassName('weeks')[0]; // lista koja ima klasu weeks (prva)
		...
		for (var i = n - 1; i >= 0; i--) {
			...
			if (link.includes('meet.google.com') && tagA.innerHTML.includes('predavanj')) {
				rez = link;	
				break;
			}
		}
